...
const s_maxLength = 1000;
const s_maxLengthName = 32;
const s_collapsedReplies = true; // collapse replies
const s_longTimestamp = false; // True for a date + time, false for just the date
let s_includeUrlParameters = true; // Makes new comment sections on pages with URL parameters when set to true  
const s_fixRarebitIndexPage = false; // If using Rarebit
...
const s_closedCommentsText = 'Comments closed for maintanance!!! Consider heading to my <a href="https://neocities.org/site/stupied-profile">Neocities Profile</a> to comment instead.'; //
// const s_closedCommentsText = 'Comments indefinitely closed.'; //
const s_reviewCommentsText ='<b>[ AWAITING APPROVAL ]</b><br><br>Thank you for commenting! Everything is moderated manually to avoid spam. Will get back to this as soon as I can.';
...
DOMPurify.setConfig(
    {
        ...
        FORBID_ATTR: ['style', 'class', 'aria-hidden', 'src', 'srcset']
    }
)
...
if (WIDGET) {
    ...
    const v_formHtml = `
        ...
        <div id="c_nameWrapper" class="c-inputWrapper">
            ...
            <input class="c-input c-nameInput" name="entry.${s_nameId}" id="entry.${s_nameId}" type="text" maxlength="${s_maxLengthName}" placeholder="Name (Required)" onchange="checkPrev('userName')" required>
        </div>
        ...
        <div id="c_websiteWrapper" class="c-inputWrapper">
            ...
            <input class="c-input c-websiteInput" name="entry.${s_websiteId}" id="entry.${s_websiteId}" type="url" pattern="https://.*" placeholder="Website (Optional)" onchange="checkPrev('userWebsite')">
        </div>

        <div id="c_textWrapper" class="c-inputWrapper">
            <label class="c-label c-textLabel" for="entry.${s_textId}">${s_textFieldLabel}</label>
            <textarea class="c-input c-textInput" name="entry.${s_textId}" id="entry.${s_textId}" rows="4" cols="50"  maxlength="${s_maxLength}" placeholder="Write your message here! - It supports SOME markdown." required></textarea>
        </div>
    ...
    if (s_commentsOpen) { c_form.innerHTML = v_formHtml }
        ...
        // moved this into the css eventually
        c_form.innerHTML = `<div id="c_closed_message"><h2>Attention!</h2><p>${s_closedCommentsText}</p></div>`;
    }
    ...
    function createComment(data) {
        ...
        if (data.Image) {
            if (data.Moderated == true) {
                ...
                // change this to be an image instead!
                let img = document.createElement('p');
                img.textContent = '[ Image attached still needs to be moderated ]';
                comment.appendChild(img);
            }
        ...
        if (commentModerated === false) {
            ...
            text.innerHTML = s_reviewCommentsText;
        }
